...
public class TexturedGlassGray extends BlockBreakable {
...
public IIcon[] icons = new IIcon[15];
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int face)
    {
        ...
        if (this == ModBlocks.TexturedGlassGray)
        {
	...
	public TexturedGlassGray(String unlocalizedName, Material material) {
		super(unlocalizedName, material.glass, false);
		this.setBlockName("TexturedGlassGray");
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < 15; i ++) {
			this.icons[i] = reg.registerIcon("wp:TexturedGlassGray-" + i);
		}
}
