...
public class FrostedGlassCyan extends BlockBreakable {
...
public IIcon[] icons = new IIcon[15];
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int face)
    {
        ...
        if (this == ModBlocks.FrostedGlassCyan)
        {
	...
	public FrostedGlassCyan(String unlocalizedName, Material material) {
		super(unlocalizedName, material.glass, false);
		this.setBlockName("FrostedGlassCyan");
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < 10; i ++) {
			this.icons[i] = reg.registerIcon("wp:FrostedGlassCyan-" + i);
		}
}
