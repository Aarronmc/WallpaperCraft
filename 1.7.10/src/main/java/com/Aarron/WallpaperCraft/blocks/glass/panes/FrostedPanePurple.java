...
public class FrostedPanePurple extends BlockStainedGlassPane
{
	private static final IIcon[] paneTextures = new IIcon[15];
    private static final IIcon[] topPaneTextures = new IIcon[15];
    
	public FrostedPanePurple() {
		super();
		this.setBlockName("FrostedPanePurple");
	...
	public void registerBlockIcons(IIconRegister reg){
        for(int i = 0; i < 15; ++i){
        	paneTextures[i] = reg.registerIcon("wp:FrostedGlassPurple-" + i);     
        }
        for(int j = 0; j < 15; ++j){	
        	topPaneTextures[j] = reg.registerIcon("wp:FrostedPaneTopPurple-" + j);
        }
    }
