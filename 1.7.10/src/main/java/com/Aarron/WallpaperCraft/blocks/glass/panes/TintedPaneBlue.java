...
public class TintedPaneBlue extends BlockStainedGlassPane
{
	private static final IIcon[] paneTextures = new IIcon[15];
    private static final IIcon[] topPaneTextures = new IIcon[15];
    
	public TintedPaneBlue() {
		super();
		this.setBlockName("TintedPaneBlue");
	...
	public void registerBlockIcons(IIconRegister reg){
        for(int i = 0; i < 15; ++i){
        	paneTextures[i] = reg.registerIcon("wp:TintedGlassBlue-" + i);     
        }
        for(int j = 0; j < 15; ++j){	
        	topPaneTextures[j] = reg.registerIcon("wp:TintedPaneTopBlue-" + j);
        }
    }
