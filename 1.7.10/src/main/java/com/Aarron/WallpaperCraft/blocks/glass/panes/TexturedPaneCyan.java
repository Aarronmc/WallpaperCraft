...
public class TexturedPaneCyan extends BlockStainedGlassPane
{
	private static final IIcon[] paneTextures = new IIcon[10];
    private static final IIcon[] topPaneTextures = new IIcon[10];
    
	public TexturedPaneCyan() {
		super();
		this.setBlockName("TexturedPaneCyan");
	...
	public void registerBlockIcons(IIconRegister reg){
        for(int i = 0; i < 10; ++i){
        	paneTextures[i] = reg.registerIcon("wp:TexturedGlassCyan-" + i);     
        }
        for(int j = 0; j < 10; ++j){	
        	topPaneTextures[j] = reg.registerIcon("wp:TexturedPaneTopCyan-" + j);
        }
    }
