...
public class FrostedPaneGray extends BlockStainedGlassPane
{
	private static final IIcon[] paneTextures = new IIcon[15];
    private static final IIcon[] topPaneTextures = new IIcon[15];
    
	public FrostedPaneGray() {
		super();
		this.setBlockName("FrostedPaneGray");
	...
	public void registerBlockIcons(IIconRegister reg){
        for(int i = 0; i < 15; ++i){
        	paneTextures[i] = reg.registerIcon("wp:FrostedGlassGray-" + i);     
        }
        for(int j = 0; j < 15; ++j){	
        	topPaneTextures[j] = reg.registerIcon("wp:FrostedPaneTopGray-" + j);
        }
    }
