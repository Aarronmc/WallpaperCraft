...
public class FrostedPaneCyan extends BlockStainedGlassPane
{
	private static final IIcon[] paneTextures = new IIcon[10];
    private static final IIcon[] topPaneTextures = new IIcon[10];
    
	public FrostedPaneCyan() {
		super();
		this.setBlockName("FrostedPaneCyan");
	...
	public void registerBlockIcons(IIconRegister reg){
        for(int i = 0; i < 10; ++i){
        	paneTextures[i] = reg.registerIcon("wp:FrostedGlassCyan-" + i);     
        }
        for(int j = 0; j < 10; ++j){	
        	topPaneTextures[j] = reg.registerIcon("wp:FrostedPaneTopCyan-" + j);
        }
    }
