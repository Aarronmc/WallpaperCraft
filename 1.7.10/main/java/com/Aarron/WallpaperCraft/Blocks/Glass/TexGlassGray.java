...
public class TexGlassGray extends BlockBreakable {
...
public IIcon[] icons = new IIcon[15];
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int face)
    {
        ...
        if (this == CoreBlock.TexGlassGray)
        {
	...
	public TexGlassGray(String unlocalizedName, Material material) {
		super(unlocalizedName, material.glass, false);
		this.setBlockName("TexGlassGray");
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < 15; i ++) {
			this.icons[i] = reg.registerIcon("wp:TexGlassGray-" + i);
		}
}
