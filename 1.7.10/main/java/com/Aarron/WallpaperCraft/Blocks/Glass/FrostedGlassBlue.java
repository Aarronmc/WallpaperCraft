...
public class FrostedGlassBlue extends BlockBreakable {
...
public IIcon[] icons = new IIcon[15];
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int face)
    {
        ...
        if (this == ModBlocks.FrostedGlassBlue)
        {
	...
	public FrostedGlassBlue(String unlocalizedName, Material material) {
		...
		this.setBlockName("FrostedGlassBlue");
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < 15; i ++) {
			this.icons[i] = reg.registerIcon("wp:FrostedGlassBlue-" + i);
		}
}
